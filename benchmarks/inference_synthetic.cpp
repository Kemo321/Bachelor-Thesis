...
constexpr float kConfThreshold = 0.10F;
...
int main()
{
    ...
    const std::string torch_path = results_dir + "/yolov1_synthetic_torch_final.pt";
    const std::string custom_path = results_dir + "/yolov1_synthetic_custom_final.pt";
    const bool have_torch = fs::exists(torch_path);
    const bool have_custom = fs::exists(custom_path);
    if (!have_torch && !have_custom)
    {
        LOG_ERROR("No Synthetic models found under {}", results_dir);
        return -1;
    }
