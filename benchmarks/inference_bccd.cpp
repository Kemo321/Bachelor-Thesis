...
int main() {
    ...
    std::cout << "==========================================\n";
    std::cout << "[BCCD INFERENCE] Start Inferencji...\n";
    std::cout << "==========================================\n";
    ...
    if (test_paths.images.empty()) {
        std::cerr << "[BLAD] Brak danych testowych!\n"; return -1;
    }
    ...
    for (size_t idx = 0; idx < images_to_process; ++idx) {
        ...
        auto final_torch = apply_nms(raw_torch, 0.60f); // Wysokie NMS dla krwinek
        ...
        auto final_custom = apply_nms(raw_custom, 0.60f); // Wysokie NMS dla krwinek
